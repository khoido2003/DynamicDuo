...
namespace Unity.DynamicDuo.Infrastructure
{
    public class MessageChannel<T> : IMessageChannel<T>
    {
        ...
        public void Publish(T message)
        {
            ...
            foreach (var messageHanlder in m_MessageHandlers)
            {
                messageHanlder?.Invoke(message);
            }
        }

        public virtual IDisposable Subscribe(Action<T> handler)
        {
            Assert.IsTrue(!IsSubscribed(handler), "Subscribe handler more than once!");
        ...
        protected bool IsSubscribed(Action<T> handler)
        {
